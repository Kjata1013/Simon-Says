...
void setup()
{
  ...
  // Check to see if lower left blue button is pressed
  if (checkButton() == CHOICE_BLUE) do_magic(); //Jump to Magic routine
...
//
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Simon Says MAGIC! - a 4 button implimentation of the classic magic button/light box.
// We are constructing what is basically a 4 On/Off switches in series to light the light.
// The buttons will act as on/off switches, and making them all ON will light the button. 
// (4 input AND gate!) This is used with some fancy magician's patter to do the trick.
// This code calls subroutines from Simon above. Much thanks to all for the open source!
// See http://www.centaur.org/simonsaysmagic/ and http://www.sparkfun.com

byte SWITCH_YBGR = CHOICE_OFF; // Switch Bit positions: 8=Yellow, 4=Blue, 2=Green, 1=Red
byte LED_YBGR = CHOICE_OFF;    // SWITCH and LED light status tracking set to 0 (off) for start

void do_magic()                // Main Entry Here <---
{
  setLEDs(CHOICE_BLUE);        // Turn on Blue LED
  toner(CHOICE_BLUE, 150);     // Play a tone
  setLEDs(CHOICE_RED | CHOICE_GREEN | CHOICE_YELLOW); // Turn on the other LEDs until you release button
  while(checkButton() != CHOICE_NONE) ;               // Wait for user to stop pressing button
  setLEDs(CHOICE_OFF);         // Turn off LEDs
  ...
  {
    byte magicchoice = wait_for_magic_button(); // See what button the user presses
    // magicchoice should now equal one of CHOICE_RED | CHOICE_GREEN | CHOICE_YELLOW | CHOICE_BLUE
    
    switch (magicchoice) {              //Which button was pressed?  Using Switch/Case logic.
      
      case CHOICE_RED:                  //Red Button
        if (!(LED_YBGR & B00001110))    //Ignoring Red LED, are any of other 3 on? If Yes, Im inhibited, do NOT do anything.
        {                               //If no other LEDS are on, I can continue my button pressed process here...
         SWITCH_YBGR = SWITCH_YBGR ^ CHOICE_RED;  //XOR - toggle RED switch bit. (If off, turn on. If on, turn off.)
         if (SWITCH_YBGR == B00001111)  //all 4 switch on? (B00001111 same as hex 0x0F)
         {
     ...
     case CHOICE_GREEN:                 //Green Button
       if (!(LED_YBGR & B00001101))     //Ignoring Green LED, are any of other 3 on? If Yes, Im inhibited, do NOT do anything.
       {                                //If no other LEDS are on, I can continue my button pressed process here...
         SWITCH_YBGR = SWITCH_YBGR ^ CHOICE_GREEN;   //XOR - toggle Green switch bit
         if (SWITCH_YBGR == B00001111)  //all 4 switch on ?
         {
     ...
     case CHOICE_BLUE:                  //Blue Button
       if (!(LED_YBGR & B00001011))     //Ignoring Blue LED, are any of other 3 on? If Yes, Im inhibited, do NOT do anything.
       {                                //If no other LEDS are on, I can continue my button pressed process here...
         SWITCH_YBGR = SWITCH_YBGR ^ CHOICE_BLUE;  //XOR - toggle Blue switch bit
         if (SWITCH_YBGR == B00001111)  //all 4 switch on ?
         {
     ...
     case CHOICE_YELLOW:                //Yellow Button
       if (!(LED_YBGR & B00000111))     //Ignoring Yellow LED, are any of other 3 on? If Yes, Im inhibited, do NOT do anything.
       {                                //If no other LEDS are on, I can continue my button pressed process here...
         SWITCH_YBGR = SWITCH_YBGR ^ CHOICE_YELLOW;  //XOR - toggle Yellow switch bit
         if (SWITCH_YBGR == B00001111)  //all 4 switch on ?
         {
  ...
  }//end while loop (cycle back to start of loop, as it is always true!)
  
}//end main Magic routine

//----------------------------------------------------------------------
// Subroutines used by Simon Says Magic!

// Wait for a button to be pressed - Magic Trick style. Mostly copied from Simon code.
byte wait_for_magic_button(void)
{
  while (true) // Loop 
  {
...
// Returns a '1' bit in the position corresponding to CHOICE_RED, CHOICE_GREEN, etc.
byte checkMagicButton(void) //Modified from Simon code above.
{
  if (digitalRead(BUTTON_RED) == 0) return(CHOICE_RED); 
  else if (digitalRead(BUTTON_GREEN) == 0) return(CHOICE_GREEN); 
  else if (digitalRead(BUTTON_BLUE) == 0) return(CHOICE_BLUE); 
